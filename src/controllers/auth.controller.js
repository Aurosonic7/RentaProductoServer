...
export const register_user = async (req, res) => {
  try {
    const { statusMessage, token } = await authModel.register_user(req.body);
    ...
    else if (statusMessage === 'Phone already exists') return res.status(409).send({ message: 'El teléfono ya está registrado' });
    else if (statusMessage === 'User registered successfully') return res.status(201).send({ message: 'Usuario registrado correctamente', token });
