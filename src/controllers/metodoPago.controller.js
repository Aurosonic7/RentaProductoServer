...
export const create_metodoPago = async (req, res) => {
  try {
    ...
    if (statusMessage === 'Pago procesado exitosamente') {
      return res.status(201).json({ message: 'Método de pago creado exitosamente', metodopago_id });
    } else if (statusMessage === 'Método de pago inválido' || statusMessage === 'Estado de pago inválido' || statusMessage === 'Renta no encontrada') {
      return res.status(400).json({ message: statusMessage });
    } else {
      return res.status(500).json({ message: 'Error desconocido durante la creación del método de pago' });
    }
  } catch (error) {
    res.status(500).json({ message: `Error durante la creación del método de pago: ${error.message}` });
  }
};

export const get_metodosPago = async (req, res) => {
  try {
...
export const get_metodoPago_by_id = async (req, res) => {
  try {
    const { metodopago_id, statusMessage, data } = await metodoPagoModel.get_metodoPago_by_id(req.params.id);
